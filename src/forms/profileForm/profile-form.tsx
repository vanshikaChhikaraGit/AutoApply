...
const [loading,setLoading] = useState(false)
        ...
        try {
            ...
            if (onAddUserProfileToDB) {
                if (onAddUserProfileToDB.status === 200) {
                    toast("Success 🎉", {
                        description: <span className='text-sm text-black'>{onAddUserProfileToDB.message}</span>
                    });
    setLoading(false)
  ...
  return (
    <div>
       <Form {...userProfileForm}>
        <form onSubmit={userProfileForm.handleSubmit(onHandleSubmit)}>
            ...
            render={({ field })=>(
                <FormItem className='m-4'>
                    <FormLabel>Upload your<span className='text-brand-500 font-semibold text-lg'>Resume</span></FormLabel>
                <FormControl>
                <div className="flex flex-col items-center justify-center w-full border-2 border-gray-500 border-dotted p-4 rounded-lg">
    {/* Upload Icon */}
    <MdAddPhotoAlternate size={35} className='text-brand-500'></MdAddPhotoAlternate>
    ...
    />
            ...
            render={({ field })=>(
                <FormItem className='m-4'>
                    <FormLabel>Upload a <FcGoogle className='mr-0' size={20}/> drive link for your resume</FormLabel>
                <FormControl>
<Input placeholder='paste drive link here' {...field}></Input>
        ...
        >
          {loading ? <>Uploading <LoadingSpinner></LoadingSpinner> </> : <>Get Started <FaArrowRightLong></FaArrowRightLong> </>}
        </Button>
             </form>
        
       </Form>
<Link href={"/onboarding/education"} className='underline text-gray-600 text-md text-center m-4 flex justify-center font-semibold'> Skip this step</Link>
    </div>
  )
}
